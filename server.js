...
app.post('/api/files', upload.any(), (req, res) => {
  const uploaded = Array.isArray(req.files) ? req.files.find((file) => file && file.fieldname === 'file') : null;
  const fileInfo = uploaded || req.file;

  if (!fileInfo) {
    return res.status(400).json({ message: 'Please upload a file.' });
  }

  const parentId = normalizeParentId(req.body && req.body.parent_id);
  const items = readImportedItems();
  const record = normalizeItem({
    id: createId('upload'),
    parent_id: parentId,
    name: String(req.body.name || fileInfo.originalname || '未命名文件').trim() || '未命名文件',
    is_dir: false,
    kind: 'file',
    url: '',
    pwd: '',
    size: String(fileInfo.size || ''),
    storage_name: fileInfo.filename,
    createdAt: new Date().toISOString(),
  });
  ...
  res.status(201).json({
    ...
    file: {
      ...record,
      path: `/api/files/${encodeURIComponent(fileInfo.filename)}`,
    },
  });
});
...
app.delete('/api/items/:id', (req, res) => {
  ...
  if (target.storage_name) {
    const filePath = path.join(uploadDir, target.storage_name);
